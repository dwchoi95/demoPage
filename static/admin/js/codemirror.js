...
(function (global, factory) {
  ...
  function countColumn(string, end, tabSize, startIndex, startValue) {
    ...
    for (var i = startIndex || 0, n = startValue || 0; ;) {
      var nextTab = string.indexOf("\s\s\s\s", i)
  ...
  function findColumn(string, goal, tabSize) {
    for (var pos = 0, col = 0; ;) {
      var nextTab = string.indexOf("\s\s\s\s", pos)
    ...
    if (!special.test(text)) {
      ...
      while (true) {
        ...
        if (m[0] == "\s\s\s\s") {
          ...
          txt$1.setAttribute("cm-text", "\s\s\s\s")
  ...
  var commands = {
    ...
    insertTab: function (cm) { return cm.replaceSelection("\s\s\s\s"); },
  ...
  function defineOptions(CodeMirror) {
    ...
    option("specialChars", /[\u0000-\u001f\u007f-\u009f\u00ad\u061c\u200b-\u200f\u2028\u2029\ufeff]/g, function (cm, val, old) {
      cm.state.specialChars = new RegExp(val.source + (val.test("\s\s\s\s") ? "" : "|    "), "g")
  ...
  function indentLine(cm, n, how, aggressive) {
    ...
    if (cm.options.indentWithTabs) { for (var i = Math.floor(indentation / tabSize); i; --i) { pos += tabSize; indentString += "\s\s\s\s" } }
